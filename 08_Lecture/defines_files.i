...
const char *pu = "\033[0;35m";
const char *cy = "\033[0;36m";
const char *ye = "\033[1;33m";
const char *og = "\033[0;33m";
const char *gn = "\033[0;32m";
const char *lgn = "\033[1;32m";
const char *lgy = "\033[0;37m";
const char *er = "\033[0m";
...
static void logo(const byte start_sp, const char *txt, const char *txt_col, const char bkg_chr,
                 const char *bkg_col){
...
# 51 "defines_files.c"
  ...
  byte lsp = (w.ws_col - 2*lthck - 2*start_sp - strlen(txt)) / 2;
  byte len = 2*lthck+2*lsp+strlen(txt);

  printf("\n%s", bkg_col);
  for (byte i = 0; i < (4*vthck+1); ++i){
    for (byte j = 0; j < start_sp; ++j){
      printf(" ");
    }
    if ((i < vthck || i > (3*vthck)) && (i != (4*vthck)/2)){
      for (byte h = 0; h < len; ++h){
        printf("%c", bkg_chr);
      }
      printf("\n");
    } else if ((i >= vthck && i <= (3*vthck)) && (i != (4*vthck)/2)) {
      for (byte l = 0; l < lthck; ++l){
        printf("%c", bkg_chr);
      }
      for (byte m = 0; m < (len - 2*lthck); ++m){
        printf(" ");
      }
      for (byte n = 0; n < lthck; ++n){
        printf("%c", bkg_chr);
      }
      printf("\n");
    } else if (i == ((4*vthck)/2)) {
      for (byte o = 0; o < lthck; ++o){
        printf("%C", bkg_chr);
      }
      for (byte p = 0; p < lsp; ++p){
        printf(" ");
      }
      printf("%s%s%s", txt_col, txt, bkg_col);
      for (byte q = 0; q < lsp; ++q){
        printf(" ");
      }
      for (byte r = 0; r < lthck; ++r){
        printf("%c", bkg_chr);
      }
      printf("\n");
    }
