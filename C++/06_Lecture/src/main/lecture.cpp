...
/* Global objects */
Term term={"Lecture-06", YE, '#', GN, 10};                                                                  // Initialize terminal UI and print responsive-title


/* Main routine to read command-line parameters (return err flg) */
static Boolean read_cl_param(C_integer &argc, char *const argv[], Real param[], C_integer &param_sz) {
  ...
  if (argc-1==param_sz) {                                                                                   // Chk for expected num of main param
    ...
    for (Integer i=1; i<argc; ++i) {                                                                        // Param scrollin' cycle (skip prg name)
      ...
      err_flg=term.chk_num_str(p[i-1], "Expecting real value, not char! [Param"+S(i)+": "+p[i-1]+"]");      // Chk each param str chr to make sure it's numeric
...
int main(const int argc, char *const argv[]) {
  ...
  /* Main code-cycle end */
  term.close_bye();                                                                                         // Close terminal UI with fbk
}
