/*
 * Author: Cristian Merli
 * Code title: Matrix library header file
 * Code version: 0.0
 * Creation date: 30/04/2022
 * Last mod. date: 30/04/2022 
 */


/* Include guards */
#pragma once                                                                                                // Avoid multiple inclusions
#ifndef _MATRIX_H_                                                                                          // Avoid multiple inclusions (old-alternative start)
#define _MATRIX_H_                                                                                          // Avoid multiple inclusions (old-alternative)


/* Dependencies */
#include "../ui/lib_ui.h"                                                                                   // Import UI (terminal I/O) library header file


/* Libraries */
//


/* Constants */
//


/* Macros */
//


/* Public functions */
//


#endif                                                                                                      // Avoid multiple inclusions (old-alternative end)
